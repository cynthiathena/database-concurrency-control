...
TxnProcessor::TxnProcessor(CCMode mode)
  ...
  // std::cout << "Test TxnProcessor" << std::endl << std::flush;
  storage_->InitStorage();

  // std::cout << "Test CPUSET" << std::endl << std::flush;
...
void TxnProcessor::RunMVCCScheduler() {
  ...
  // [For now, run serial scheduler in order to make it through the test
  // suite]
  // std::cout << "RunMVCCScheduler" << std::endl << std::flush;
  // int count = 0;
  while (tp_.Active()){
    Txn *txn;
    // count += 1;
    if (txn_requests_.Pop(&txn)) {
      ...
      // std::cout << "Runtask : " << count << std::endl << std::flush;
      // Start txn running in its own thread.
      tp_.RunTask(new Method<TxnProcessor, void, Txn*>(
                  this,
...
void TxnProcessor::MVCCExecuteTxn(Txn* txn){
  ...
  // std::cout << "Read all datas" << std::endl << std::flush;  
    ...
    // completed_txns_.Push(txn);
    // std::cout << "Acquire all locks" << std::endl << std::flush;  
    //Acquire all locks for keys in the write_set_
    MVCCLockWriteKeys(txn);

    // std::cout << "Check Write" << std::endl << std::flush;  
    //Call MVCCStorage::CheckWrite method to check all keys in the write_set_
    if (MVCCCheckWrites(txn)){ //If (each key passed the check)
      // std::cout << "Apply Write" << std::endl << std::flush;  
      // Apply the writes
      ApplyWrites(txn);

      // std::cout << "Release Locks after Applying Writes" << std::endl << std::flush;
      ...
      // std::cout << "Didnt pass the check" << std::endl << std::flush;
      // Release all locks for keys in the write_set_
      MVCCUnlockWriteKeys(txn);
